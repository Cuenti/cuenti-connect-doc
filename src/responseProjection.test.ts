...
describe('projectResponseExample', () => {
  it('removes and restores selected header groups', () => {
    const endpoint = registry.endpoints.find(
      ({ contractId }) => contractId === 'buscarProductosCatalogo',
    )!;
  ...
  it('shows every authorized product group when all groups are requested', () => {
    ...
    const response = projectResponseExample(endpoint, endpoint.requestExample) as {
  ...
  it('projects detail groups independently', () => {
    const endpoint = {
      ...registry.endpoints.find(
        ({ contractId }) => contractId === 'buscarDocumentosComerciales',
      )!,
      responseExample: {
        pagina: 0,
        documentos: [
          {
            ...
            detalle: [{ producto: { id_producto: 25, nombre: 'Producto', sku: 'SKU-25' } }],
          },
        ],
      },
    };
    ...
    expect(response.documentos[0]).toHaveProperty('detalle');
    expect(response.documentos[0].detalle).toEqual([{ producto: { id_producto: 25, nombre: 'Producto', sku: 'SKU-25' } }]);
  });
});
